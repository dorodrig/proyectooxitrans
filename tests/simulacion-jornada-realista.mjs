// ================================================
// TEST: SIMULACIÓN JORNADA LABORAL REALISTA
// 5 empleados × 1 día × 4 eventos = 20 registros
// Fecha: 7 de octubre 2025 (ayer)
// ================================================

const API_BASE = 'http://localhost:3001/api';

// Solo 5 usuarios seleccionados para garantizar éxito
const empleados = [
...
// Coordenadas GPS por regional (precisas para cada sede)
...
// ================================================
// FUNCIONES OPTIMIZADAS Y REALISTAS
// ================================================
...
async function loginUsuario(documento, password) {
  try {
    ...
    console.log(`   ✅ Login exitoso para ${documento}`);
    ...
    console.log(`   ⚠️ No se encontró token en respuesta`);
    return null;
  } catch (error) {
...
async function ejecutarSimulacionRealista() {
  console.log('🎯 SIMULACIÓN JORNADA LABORAL REALISTA');
  console.log('='.repeat(70));
  console.log(`👥 Empleados seleccionados: ${empleados.length}`);
  console.log(`📅 Fecha objetivo: 7 de octubre 2025 (ayer)`);
  console.log(`⏰ Horarios realistas: 7:30 AM → 12:00 PM → 1:00 PM → 5:30 PM`);
  ...
  console.log(`✅ Secuencia lógica garantizada con validaciones`);
  ...
  console.log('\n🎉 SIMULACIÓN JORNADA LABORAL COMPLETADA');
  console.log('='.repeat(70));
  ...
  console.log('\n✨ SIMULACIÓN JORNADA LABORAL FINALIZADA ✨');
}
...
console.log('⏰ Horarios realistas: 7:30 AM → 12:00 PM → 1:00 PM → 5:30 PM');
console.log('🔄 Secuencia lógica garantizada con validaciones');
console.log('✅ Datos coherentes para análisis de horas trabajadas');
console.log('⏳ Tiempo estimado: 3-4 minutos\n');

setTimeout(() => {
  ejecutarSimulacionRealista().catch(error => {
    console.error('💥 Error fatal en simulación realista:', error);
  });
}, 2000);